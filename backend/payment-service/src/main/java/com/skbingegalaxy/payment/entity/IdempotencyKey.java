...
/**
 * Stripe-style Idempotency-Key record.
 *
 * <p>Composite PK (key, method, path, userId) prevents collisions when a client
 * reuses a key across different endpoints or users. The stored SHA-256 hash
 * lets us detect "same key, different payload" — the classic client bug that
 * weak dedupe would silently accept.
 */
@Entity
...
public class IdempotencyKey {
    ...
    /** Null for unauthenticated flows (there are none today, kept nullable for safety). */
    @Id
    @Column(name = "user_id")
    private Long userId;
