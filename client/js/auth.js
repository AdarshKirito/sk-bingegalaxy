...
const API_BASE_URL = isLiveServer ? 'http://127.0.0.1:5000' : '';
const APP_BASE_URL = isLiveServer ? 'http://127.0.0.1:5000' : '';
...
document.getElementById('loginForm')?.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (result.success) {
            // Customer login page should not authenticate admins.
            if (result.user.role === 'admin') {
                showAlert('Admin account detected. Please use the Admin Login page.');
                return;
            }
...
const checkAuth = () => {
    ...
    const isUserLoginPage = currentPath.includes('/login') && !isAdminLoginPage;
    const isAnyLoginPage = isAdminLoginPage || isUserLoginPage;
    
    // If user is logged in
    if (token && user.role) {
        // Portal separation on login pages:
        // - user login page only auto-redirects customers
        // - admin login page only auto-redirects admins
        if (isAnyLoginPage) {
            if (isUserLoginPage && user.role === 'customer') {
                goTo('/dashboard');
                return;
            }
            ...
            // Keep user on current login page if role does not match that portal.
            return;
        }
        // If user is on wrong dashboard type, redirect to correct one
        else if (user.role === 'admin' && currentPath.includes('dashboard') && !currentPath.includes('admin')) {
