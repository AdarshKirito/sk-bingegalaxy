...
export default function StepCustomer({
  ...
  const [newCust, setNewCust] = useState({ firstName: '', lastName: '', email: '', phone: '', password: '', guest: false });
  ...
  const handleCreateCustomer = async () => {
    ...
    // ── Walk-in guest ("no email / not opting for email") ──
    // Only first + last name required; phone optional; no credentials issued.
    if (newCust.guest) {
      if (!newCust.lastName.trim()) { toast.error('Last name is required for a guest profile'); return; }
      setCreating(true);
      try {
        ...
        setShowCreateForm(false);
        setTempNotice(null);
        setNewCust({ firstName: '', lastName: '', email: '', phone: '', password: '', guest: false });
        toast.success('Guest profile created — no login credentials were issued.');
      } catch (err) {
        toast.error(err.userMessage || err.response?.data?.message || 'Failed to create guest profile. Please try again.');
      } finally {
        setCreating(false);
      }
      return;
    ...
    if (!newCust.email.trim()) { toast.error('Email is required to create a customer (or tick "No email" for a walk-in guest)'); return; }
    ...
    try {
      ...
      // Desk registration = same benefits as self-signup: enroll them in the
      // rewards program right away (welcome bonus on first enrollment).
      // Best-effort — a failure self-heals on their first booking.
      if (created?.id) {
        try {
          const enr = await loyaltyV2.adminEnrollCustomer(created.id);
          if (enr && !enr.alreadyEnrolled) {
            const pts = Number(enr.welcomeBonusPoints || 0);
            toast.info(pts > 0
              ? `⭐ Enrolled in rewards — ${pts.toLocaleString()} welcome bonus points credited (member ${enr.memberNumber})`
              : `⭐ Enrolled in the rewards program (member ${enr.memberNumber})`);
          }
        } catch { /* silent: lazy enrollment on first booking covers this */ }
      }
    } catch (err) {
      ...
      {selectedCustomer ? (
        <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem' }}>
          <div>
            ...
            {selectedCustomer.guest && <span className="badge badge-warning" style={{ marginLeft: '0.5rem', fontSize: '0.72rem' }}>Guest — no account</span>}
            <br />
            <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
              {selectedCustomer.guest ? 'No email (walk-in guest)' : selectedCustomer.email}
            </span>
        ...
        <div className="card" style={{ padding: '1.25rem' }}>
          <div style={{ position: 'relative' }}>
            <input value={custQuery} onChange={e => setCustQuery(e.target.value)}
              type="search" name="customer-search" autoComplete="off"
          ...
          {showCreateForm && (
            <div style={{ marginTop: '0.75rem', padding: '1rem', background: 'var(--bg-input)', borderRadius: 'var(--radius-sm)' }}>
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', marginBottom: '0.75rem', cursor: 'pointer' }}>
                <input
                  type="checkbox"
                  checked={newCust.guest}
                  onChange={(e) => setNewCust({ ...newCust, guest: e.target.checked, email: '', password: '' })}
                  style={{ marginTop: '0.2rem' }}
                  aria-label="Walk-in guest with no email"
                />
                <span style={{ fontSize: '0.85rem' }}>
                  <strong>No email / not opting for email (walk-in guest)</strong>
                  <br />
                  <span style={{ color: 'var(--text-muted)', fontSize: '0.78rem' }}>
                    Only first &amp; last name required. No password or email is created — the guest
                    can't log in, but you can book, rebook and manage reservations for them.
                  </span>
                </span>
              </label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                ...
                <div><label style={labelStyle}>Last Name{newCust.guest ? ' *' : ''}</label><input style={inputStyle} value={newCust.lastName} onChange={e => setNewCust({ ...newCust, lastName: e.target.value })} aria-label="Last name" /></div>
                {!newCust.guest && (
                  <div><label style={labelStyle}>Email *</label><input type="email" name="new-customer-email" autoComplete="off" style={inputStyle} value={newCust.email} onChange={e => setNewCust({ ...newCust, email: e.target.value })} aria-label="Email" /></div>
                )}
                <div style={newCust.guest ? { gridColumn: '1 / -1' } : undefined}><label style={labelStyle}>Phone{newCust.guest ? ' (optional)' : ''}</label><input style={inputStyle} value={newCust.phone} onChange={e => setNewCust({ ...newCust, phone: e.target.value })} aria-label="Phone" placeholder={newCust.guest ? 'Optional — for front-desk contact only' : 'For SMS delivery of the temp password'} /></div>
                <div style={{ gridColumn: '1 / -1', display: newCust.guest ? 'none' : undefined }}>
                  <label style={labelStyle}>Temporary Password *</label>
                  <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                    <input type={showPw ? 'text' : 'password'} name="new-customer-temp-password" style={{ ...inputStyle, fontFamily: 'monospace', letterSpacing: '0.04em' }}
                      value={newCust.password} onChange={e => setNewCust({ ...newCust, password: e.target.value })}
                      aria-label="Temporary password" placeholder="Click Generate" autoComplete="new-password" />
              ...
              <button className="btn btn-primary btn-sm" style={{ marginTop: '0.75rem' }} onClick={handleCreateCustomer} disabled={creating}>
                {creating ? 'Creating…' : newCust.guest ? 'Create Guest Profile' : 'Create Customer'}
              </button>
            </div>
          )}
        </div>
