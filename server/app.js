...
const mongoUris = [
    ...
    'mongodb://127.0.0.1:27017/sk-binge-galaxy'
].filter(Boolean);
...
const initializeApp = async () => {
    try {
        // Connect to MongoDB with fallback URIs.
        let connected = false;
        ...
        for (const uri of mongoUris) {
            try {
                ...
                lastDbError = dbError;
                console.warn(`⚠️  MongoDB connection failed for URI: ${uri}`);
            }
        }

        if (!connected) {
            throw lastDbError || new Error('All MongoDB connection attempts failed');
        }
